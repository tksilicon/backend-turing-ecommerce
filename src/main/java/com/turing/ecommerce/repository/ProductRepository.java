...
public interface ProductRepository extends PagingAndSortingRepository<Product, Integer> {
	...
	@Query(value = "select new com.turing.ecommerce.DTO.ProductCategoryDTO(pp.name, pp.productId, pp.description, pp.price,"
	...
	List<ProductCategoryDTO> productSearchCategory(@Param("categoryId") Integer categoryId,
			@Param("description_length") Integer description_length, Pageable pageable);

	

	@Query(value = "select new com.turing.ecommerce.DTO.ProductCategoryDTO(f.name, f.productId, f.description, f.price, "
	...
	List<ProductCategoryDTO> productSearchDepartment(@Param("departmentId") Integer departmentId,
			@Param("description_length") Integer description_length, Pageable pageable);

	
	@Query(value ="SELECT new com.turing.ecommerce.DTO.ProductDetailsDTO(p.productId, p.name, p.description, p.price, p.discountedPrice,"
			+ " p.image, p.image2) FROM Product p WHERE  p.productId = :productId")
	Optional<ProductDetailsDTO> getProductDetails(@Param("productId") Integer productId);
	
	 @Query(value ="Select new com.turing.ecommerce.DTO.ProductLocationsDTO(b.categoryId, b.name, a.departmentId, a.name) "
	 ...
	 Optional<ProductLocationsDTO> getProductLocations(@Param("productId") Integer productId);
